...
const sheetVariants = cva(
  "fixed z-50 gap-4 bg-[#1a1a1f] p-6 shadow-2xl transition ease-in-out data-[state=open]:animate-in data-[state=closed]:animate-out data-[state=closed]:duration-300 data-[state=open]:duration-500",
  {
    variants: {
      side: {
  ...
  ({ side = "right", className, children, ...props }, ref) => (
    <SheetPortal>
      ...
      <SheetPrimitive.Content ref={ref} className={cn(sheetVariants({ side }), className)} {...props}>
        {children}
        <SheetPrimitive.Close className="absolute right-4 top-4 rounded-xl p-1 text-slate-400 opacity-70 ring-offset-[#0d0d0f] transition-all hover:opacity-100 hover:text-white hover:bg-white/5 focus:outline-none focus:ring-2 focus:ring-violet-500/20 focus:ring-offset-2 disabled:pointer-events-none">
          <X className="h-4 w-4" />
