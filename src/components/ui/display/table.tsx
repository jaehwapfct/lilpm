...
const TableFooter = React.forwardRef<HTMLTableSectionElement, React.HTMLAttributes<HTMLTableSectionElement>>(
  ({ className, ...props }, ref) => (
    <tfoot ref={ref} className={cn("border-t border-white/10 bg-[#1a1a1f]/50 font-medium [&>tr]:last:border-b-0", className)} {...props} />
  ),
);
...
const TableRow = React.forwardRef<HTMLTableRowElement, React.HTMLAttributes<HTMLTableRowElement>>(
  ({ className, ...props }, ref) => (
    <tr
      ref={ref}
      className={cn("border-b border-white/10 transition-colors data-[state=selected]:bg-violet-500/10 hover:bg-white/5", className)}
      {...props}
    />
  ),
);
