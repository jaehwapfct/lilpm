        ...
        <div className="flex items-center justify-between">
          ...
          <div className="flex items-center gap-4 text-sm">
            <div className="flex items-center gap-1.5">
              ...
              <span className="text-muted-foreground">{t('cycles.ideal')}</span>
            </div>
            <div className="flex items-center gap-1.5">
              <div className="h-3 w-3 rounded-full bg-green-500" />
              <span className="text-muted-foreground">{t('cycles.actual')}</span>
            </div>
          </div>
        </div>
      </CardHeader>
      <CardContent>
        ...
        <div className="grid grid-cols-3 gap-4 mb-4">
          <div className="text-center p-3 rounded-lg bg-muted/50">
            <div className="text-2xl font-semibold text-foreground">{issues.length}</div>
            <div className="text-xs text-muted-foreground">{t('cycles.totalIssues')}</div>
          </div>
          <div className="text-center p-3 rounded-lg bg-muted/50">
            <div className="text-2xl font-semibold text-green-500">{completedIssues}</div>
            <div className="text-xs text-muted-foreground">{t('cycles.completed')}</div>
          </div>
          <div className="text-center p-3 rounded-lg bg-muted/50">
            <div className="text-2xl font-semibold text-orange-500">{remainingIssues}</div>
            <div className="text-xs text-muted-foreground">{t('cycles.remaining')}</div>
          </div>
        </div>
        ...
        <div className="mt-4 pt-4 border-t border-border flex items-center justify-between text-sm">
          <div className="flex items-center gap-2 text-muted-foreground">
            ...
            <span>{t('cycles.dailyVelocity')}: <span className="font-medium text-foreground">{velocity}</span> {t('cycles.issuesPerDay')}</span>
          </div>
          {remainingIssues > 0 && (
